...
- [x] Topic ACL (`AccessPolicy`) — landed as `src/security/acl.rs`'s
  `AccessPolicy`, `Rule`, and `Permission`. Direct port of go-DDS's
  `security.AccessPolicy` (`github.com/SoundMatt/go-DDS`,
  `security/access.go`): `AccessPolicy::new` takes an ordered list of
  `Rule { pattern, allow }` pairs; `can_read`/`can_write` walk the rules
  in declaration order and return whether the *first* rule whose
  `pattern` matches the topic name grants the corresponding
  `Permission::READ`/`WRITE` bit — first-match-wins, and a topic matching
  no rule at all is denied every permission (default-deny). Unlike
  `SecurityPlugin`'s three implementations above, `AccessPolicy` is an
  orthogonal authorization mechanism, not a payload-seal/open transform,
  and does not implement `SecurityPlugin`. Pattern matching is a
  byte-exact port of Go stdlib's `path.Match` shell-glob algorithm
  (ported directly from `$GOROOT/src/path/match.go`, not reimplemented
  from its doc-comment summary) — the same algorithm go-DDS's
  `AccessPolicy.allows` delegates to via Go's standard library — so
  `'*'`/`'?'`/`'[...]'`/`'\\'` syntax and malformed-pattern classification
  (unterminated or empty character class, dangling `-`) match go-DDS
  exactly; a rule with a malformed pattern is silently skipped rather
  than treated as a match or surfaced as an error, matching go-DDS's own
  `allows`. As with the other three landed items in this module tree,
  wiring `AccessPolicy` checks into
  `rtps::participant::RtpsParticipant`'s write/receive paths remains
  deferred — out of scope until a concrete caller need arises. Adds
  REQ-SEC-024/REQ-SEC-025 (continuing after REQ-SEC-022/023) with full
  `fusa:req`/`fusa:test` traceability. Zero `unsafe`
  (REQ-ASIL-002/REQ-MEM-001); no `.unwrap()` on any user-visible
  (non-test) path (REQ-ASIL-003). Unit-tested per this crate's
  established per-module convention: go-DDS reference vectors for the
  underlying glob algorithm (33 `(pattern, name)` pairs, generated by
  running a small Go program against a fresh go-DDS clone's own
  `path.Match`, including malformed-pattern/error cases), a 1:1 port of
  every test in go-DDS's own `access_test.go` (exact match, read-write,
  single-segment and top-level glob matching, no-match/empty-policy
  deny-all, first-match-wins, malformed-pattern-skipped), plus
  additional coverage (a `Vec<Rule>` constructor call, the
